...
// PHASE 2 UPGRADE: Enhanced TradeRecordV8 with full decision context
...
// Legacy TradeRecord for backward compatibility
struct TradeRecord {
   datetime timestamp;
   ENUM_ORDER_TYPE type;
   double entryPrice;
...
// PHASE 2 UPGRADE: Two-tier memory system
TradeRecordV8 tradeHistoryV8[];     // Episodic memory (enhanced records)
int tradeHistoryCount = 0;

// Legacy array for backward compatibility with existing functions
TradeRecord tradeHistory[];
int tradeHistoryLegacyCount = 0;

QTableEntry qTable[];
int qTableSize = 0;

// PHASE 2 UPGRADE: Beta-Bernoulli Bayesian state (alpha/beta counts instead of raw probs)
struct BayesianBetaState {
   int alpha;  // wins + 1
   int beta;   // losses + 1
   double posteriorMean;
};
BayesianBetaState bayesState[5];  // Index 0=AUTO, 1=SCALP, 2=SWING, 3=BREAKOUT, 4=REVERSAL

BayesianStrategyProb bayesProbs;  // Legacy struct for backward compat
...
// CSV file paths for learning - PHASE 2 UPGRADE: versioned filenames
string tradeLogCSV = "GoldHunter_TradeLog_v8.csv";
string qTableCSV = "GoldHunter_QTable_v8.csv";
...
// PHASE 2: Get session bucket for Q-state encoding (0=Asian, 1=London, 2=NY, 3=Overlap)
int GetSessionBucket()
{
...
// PHASE 2: Q-State Encoder - converts AgentContext to discrete state index
// State space: 5(regime) × 3(RSI) × 3(ADX) × 2(EMA) × 4(session) = 360 states
int QState_Encode(AgentContext &ctx)
{
   ...
   int rsiBucket = 1;  // default middle
   if(ctx.rsiValue < 35) rsiBucket = 0;      // Oversold
   else if(ctx.rsiValue > 65) rsiBucket = 2; // Overbought
   
   int adxBucket = 1;  // default moderate
   if(ctx.adxValue < 20) adxBucket = 0;      // Weak trend
   else if(ctx.adxValue > 35) adxBucket = 2; // Strong trend
   ...
   // Formula: regime + 5*(rsi + 3*(adx + 3*(ema + 2*session)))
   return regimeBucket + 5*(rsiBucket + 3*(adxBucket + 3*(emaBucket + 2*sessionBucket)));
}

// PHASE 2: Beta-Bernoulli Bayesian Update
void Bayes_UpdatePosterior(int strategy, bool wasWin)
{
   // Ensure strategy index is valid (0-4)
   if(strategy < 0 || strategy > 4) return;
   
   // Beta-Bernoulli conjugate update
   // alpha = wins + 1, beta = losses + 1
   if(wasWin) {
      bayesState[strategy].alpha++;
   } else {
      bayesState[strategy].beta++;
   }
   
   // Compute posterior mean: alpha / (alpha + beta)
   int total = bayesState[strategy].alpha + bayesState[strategy].beta;
   if(total > 0) {
      bayesState[strategy].posteriorMean = (double)bayesState[strategy].alpha / total;
   }
}

// PHASE 2: Get posterior mean for a strategy
double Bayes_GetPosteriorMean(int strategy)
{
   if(strategy < 0 || strategy > 4) return 0.5;
   return bayesState[strategy].posteriorMean;
}

// PHASE 2: Select best strategy based on posterior mean
int Bayes_SelectBestStrategy(AgentContext &ctx)
{
   int bestStrategy = 0;
   double bestPosterior = 0.0;
   
   for(int i = 0; i < 5; i++) {
      if(bayesState[i].posteriorMean > bestPosterior) {
         bestPosterior = bayesState[i].posteriorMean;
         bestStrategy = i;
      }
   }
...
// PHASE 2: Initialize Bayesian state with uniform priors (alpha=1, beta=1)
void Bayes_Initialize()
{
   for(int i = 0; i < 5; i++) {
      bayesState[i].alpha = 1;
      bayesState[i].beta = 1;
      bayesState[i].posteriorMean = 0.5;
   }
}

// PHASE 2: Working Memory Update - maintains ring buffers and computes regime stability
void WorkingMemory_Update(AgentContext &ctx)
{
   // Note: recentContexts ring buffer removed to avoid forward declaration issues
   // We still maintain regime and confidence buffers for pattern detection
   
   // Update recent regimes ring buffer (last 20 regimes)
   workingMem.recentRegimes[workingMem.regimeHead] = ctx.marketRegime;
   workingMem.regimeHead = (workingMem.regimeHead + 1) % 20;
   
   // Update recent confidences
   workingMem.recentConfidences[workingMem.confHead] = ctx.aggregateConfidence;
   workingMem.confHead = (workingMem.confHead + 1) % 20;
   
   // Compute regime stability score
   int currentRegime = ctx.marketRegime;
   int matchCount = 0;
   for(int i = 0; i < 20; i++) {
      if(workingMem.recentRegimes[i] == currentRegime) matchCount++;
   }
   workingMem.regimeStabilityScore = (double)matchCount / 20.0;
}

// PHASE 2: Episodic Memory Persist - writes TradeRecordV8 to CSV
void EpisodicMemory_Persist(TradeRecordV8 &rec)
{
   int fileHandle = FileOpen(tradeLogCSV, FILE_CSV|FILE_WRITE|FILE_SHARE_READ|FILE_ANSI);
   if(fileHandle == INVALID_HANDLE) {
      Print("ERROR: Could not open ", tradeLogCSV, " for writing");
      return;
   }
   ...
   if(FileSize(fileHandle) == 0) {
      FileWrite(fileHandle, 
         "timestamp,ticket,type,entryPrice,exitPrice,profit,profitPips,"
         "slDistance,tpDistance,lotsUsed,barsHeld,regime,strategy,confidence,"
         "buyScore,sellScore,reasons,pipelineTrace,atrAtEntry,adxAtEntry,rsiAtEntry,"
         "spreadAtEntry,sessionAtEntry,wasWin,maePoints,mfePoints,exitReason");
   }
   ...
   // Write record
   FileWrite(fileHandle,
      rec.timestamp, rec.ticket, (int)rec.type, rec.entryPrice, rec.exitPrice,
      rec.profit, rec.profitPips, rec.slDistance, rec.tpDistance, rec.lotsUsed,
      rec.barsHeld, rec.regime, rec.strategy, rec.confidence,
      rec.buyScore, rec.sellScore, rec.reasons, rec.pipelineTrace,
      rec.atrAtEntry, rec.adxAtEntry, rec.rsiAtEntry, rec.spreadAtEntry,
      rec.sessionAtEntry, rec.wasWin, rec.maePoints, rec.mfePoints, rec.exitReason);
   
   FileClose(fileHandle);
}

// PHASE 2: Episodic Memory Load - reads last N records from CSV
bool EpisodicMemory_LoadLast(int n, TradeRecordV8 &out[])
{
   int fileHandle = FileOpen(tradeLogCSV, FILE_CSV|FILE_READ|FILE_SHARE_WRITE|FILE_ANSI);
   if(fileHandle == INVALID_HANDLE) {
      Print("INFO: No existing ", tradeLogCSV, " found (first run)");
      return false;
   }
   ...
   // Read all records into temporary array
   TradeRecordV8 temp[];
   ArrayResize(temp, 0);
   ...
   // Copy last N records
   int startIdx = MathMax(0, ArraySize(temp) - n);
   int copyCount = ArraySize(temp) - startIdx;
   
   ArrayResize(out, copyCount);
   for(int i = 0; i < copyCount; i++) {
      out[i] = temp[startIdx + i];
   }
   
   return true;
}

// PHASE 2: Q-Table Save to CSV
void QTable_Save()
{
   int fileHandle = FileOpen(qTableCSV, FILE_CSV|FILE_WRITE|FILE_SHARE_READ|FILE_ANSI);
   if(fileHandle == INVALID_HANDLE) {
      Print("ERROR: Could not open ", qTableCSV, " for writing");
      return;
   }
   
   // Write header
   FileWrite(fileHandle, "key,qBuy,qSell,qHold,visits,lastUpdate");
   
   // Write all entries
   for(int i = 0; i < qTableSize; i++) {
      FileWrite(fileHandle,
         i, qTable[i].qValues[0], qTable[i].qValues[1], qTable[i].qValues[2],
         qTable[i].visitCount, qTable[i].lastUpdate);
   }
   
   FileClose(fileHandle);
}

// PHASE 2: Q-Table Load from CSV
void QTable_Load()
{
   int fileHandle = FileOpen(qTableCSV, FILE_CSV|FILE_READ|FILE_SHARE_WRITE|FILE_ANSI);
   if(fileHandle == INVALID_HANDLE) {
      Print("INFO: No existing ", qTableCSV, " found (first run)");
      return;
   }
   ...
   while(!FileIsEnding(fileHandle)) {
      ...
      int idx = qTableSize;
      ArrayResize(qTable, qTableSize + 1);
      qTableSize++;
   }
   
   FileClose(fileHandle);
   Print("Loaded ", qTableSize, " Q-table entries from ", qTableCSV);
}

// PHASE 2: Initialize Session Working Memory
void SessionWorkingMemory_Init()
{
   ZeroMemory(workingMem);
   workingMem.sessionStartTime = TimeCurrent();
   workingMem.sessionPeakEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   workingMem.regimeStabilityScore = 1.0;  // Start assuming stable
}

// PHASE 2: Update session statistics on trade close
void SessionWorkingMemory_OnTradeClose(double profit, bool wasWin)
{
   if(profit > 0) {
      workingMem.sessionWins++;
      workingMem.sessionRealizedPnL += profit;
   } else {
      workingMem.sessionLosses++;
      workingMem.sessionRealizedPnL += profit;
      workingMem.sessionConsecLosses++;
   }
   
   // Update peak equity
   double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(currentEquity > workingMem.sessionPeakEquity) {
      workingMem.sessionPeakEquity = currentEquity;
   }
}
