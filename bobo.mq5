--- god_AGI_v8_Final_Complete.mq5 (原始)
//+------------------------------------------------------------------+
...
struct QTableEntry {
   ...
   double eligibility[3]; // PHASE 5: Eligibility traces for Q(λ)
   int visitCount;
   datetime lastUpdate;
   double avgReward;      // PHASE 5: Rolling average reward for this state
};
...
//==========================================================================
//  PHASE 4 & 5: ADDITIONAL GLOBAL STATE FOR ACTIVE INFERENCE
//==========================================================================
...
input bool   UseNeuralNetwork         = true;   // [PH5] ENABLED: NN inference + online learning
...
int OnInit()
{
   ...
   // PHASE 5: Load Bayesian priors from GlobalVariables (survives restarts)
   ...
   // PHASE 5: Load Q-table (binary) + NN weights
   QTable_Load();
   NN_LoadWeights_Binary();

   // PHASE 4: Audit startup
   AuditLog_Write("INIT", "GoldHunter AGI v8.4 Final started");

   Print("GoldHunter UHF AGI v8.4 Final initialized. Symbol: ", Symbol(),
...
{
   // PHASE 5b-E: Call OnDeinit_AGI FIRST to flush all learning state
   OnDeinit_AGI(reason);
...
void OnTick()
{
   ...
   // PHASE 5b-A: Replace old ExecuteSignal with full OODA AgentLoop
   AgentLoop();
}
                        ...
                        const MqlTradeResult      &result)
{
   ...
   if(isClose || netProfit != 0.0) {
      ...
      // PHASE 5: Feed reflection pipeline for online learning
      reflectPending    = true;
...
//==========================================================================
//  PHASE 5: NEURAL NETWORK & REINFORCEMENT LEARNING ENHANCEMENTS
//==========================================================================

//+------------------------------------------------------------------+
//| RandomGaussian — Box-Muller transform for normal distribution    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Xavier_InitWeights — Initialize NN with Xavier initialization    |
//+------------------------------------------------------------------+
void Xavier_InitWeights()
{
   ...
   Print("[PH5] Neural Network initialized with Xavier weights");
}
...
void NN_BuildInputVector(double &inputs[], AgentContext &ctx)
{
   ...
   inputs[1] = (adxValue - 30.0) / 30.0;                           // ADX centered at 30
   inputs[2] = (emaFast - emaSlow) / (atrValue + 0.0001);          // EMA spread in ATR units
   inputs[3] = (macdMain - macdSignal) / (atrValue + 0.0001);      // MACD cross in ATR units
...
void NN_Backprop(const double &inputs[], double target, double learningRate)
{
   ...
   // Forward pass (recompute for backprop)
   for(int j = 0; j < 15; j++) {
      double sum = nnLayer.hiddenBias[j];
      for(int i = 0; i < 10; i++) {
         sum += inputs[i] * nnLayer.weights[i][j];
   ...
   double output = MathTanh(out);

   // Output error: target - output
   double outputError = target - output;

   // Gradient clipping: clip delta to [-1.0, +1.0]
   double outputDelta = MathMax(-1.0, MathMin(1.0, outputError * (1.0 - output * output)));

   // Update output weights with L2 regularization
   for(int j = 0; j < 15; j++) {
      double grad = outputDelta * hidden[j];
      nnLayer.outputWeights[j] += learningRate * (grad - 0.001 * nnLayer.outputWeights[j]);
   }
   nnLayer.outputBias += learningRate * outputDelta;

   // Hidden layer deltas
   for(int j = 0; j < 15; j++) {
      double hiddenError = outputDelta * nnLayer.outputWeights[j];
      hiddenDelta[j] = hiddenError * (1.0 - hidden[j] * hidden[j]);  // tanh derivative
   }

   // Update hidden weights with L2 regularization
   for(int i = 0; i < 10; i++) {
      for(int j = 0; j < 15; j++) {
         double grad = hiddenDelta[j] * inputs[i];
         nnLayer.weights[i][j] += learningRate * (grad - 0.001 * nnLayer.weights[i][j]);
      }
   }

   // Update hidden biases
   for(int j = 0; j < 15; j++) {
      nnLayer.hiddenBias[j] += learningRate * hiddenDelta[j];
   }
}

//+------------------------------------------------------------------+
//| NN_SaveWeights — Save NN weights to binary file                  |
//+------------------------------------------------------------------+
void NN_SaveWeights()
{
   string filename = "GoldHunter_NN_Weights_v8.bin";
   int handle = FileOpen(filename, FILE_WRITE|FILE_BIN);

   if(handle == INVALID_HANDLE) {
      Print("[PH5] Failed to save NN weights: ", GetLastError());
      return;
   }

   // Write struct as binary
   FileWriteArray(handle, nnLayer.weights);
   FileWriteArray(handle, nnLayer.hiddenBias);
   FileWriteArray(handle, nnLayer.outputWeights);
   FileWriteDouble(handle, nnLayer.outputBias);

   FileClose(handle);
   Print("[PH5] NN weights saved to ", filename);
}

//+------------------------------------------------------------------+
//| NN_LoadWeights_Binary — Load NN weights from binary file         |
//+------------------------------------------------------------------+
bool NN_LoadWeights_Binary()
{
   string filename = "GoldHunter_NN_Weights_v8.bin";
   int handle = FileOpen(filename, FILE_READ|FILE_BIN);

   if(handle == INVALID_HANDLE) {
      Print("[PH5] NN weights file not found, using Xavier initialization");
      Xavier_InitWeights();
      return false;
   }

   // Read struct as binary
   FileReadArray(handle, nnLayer.weights);
   FileReadArray(handle, nnLayer.hiddenBias);
   FileReadArray(handle, nnLayer.outputWeights);
   nnLayer.outputBias = FileReadDouble(handle);

   FileClose(handle);

   // Validate weights (check for NaN/Inf)
   bool valid = true;
   ...
   if(!valid) {
      Print("[PH5] NN weights corrupted, reinitializing with Xavier");
      Xavier_InitWeights();
      return false;
   }
...
//+------------------------------------------------------------------+
//| QLearning_UpdateWithTrace — Q(λ) update with eligibility traces  |
//+------------------------------------------------------------------+
void QLearning_UpdateWithTrace(int stateIdx, int action, double reward, int nextStateIdx)
{
   if(!UseReinforcementLearning) return;
   if(stateIdx < 0 || stateIdx >= qTableSize) return;
   if(nextStateIdx < 0 || nextStateIdx >= qTableSize) return;
   ...
   // TD error: δ = reward + γ * max(Q[s']) - Q[s,a]
   double currentQ = qTable[stateIdx].qValues[action];
   double tdError = reward + Q_DISCOUNT_FACTOR * maxNextQ - currentQ;

   // Increment eligibility for current state-action
   qTable[stateIdx].eligibility[action] += 1.0;

   // Update all states in table using eligibility traces
   for(int s = 0; s < qTableSize; s++) {
      for(int a = 0; a < 3; a++) {
         double elig = qTable[s].eligibility[a];
         if(MathAbs(elig) > 0.001) {  // Sparsity threshold
            qTable[s].qValues[a] += Q_LEARNING_RATE * tdError * elig;
            qTable[s].eligibility[a] *= Q_DISCOUNT_FACTOR * Q_LAMBDA;  // Decay: γλ
         } else {
            qTable[s].eligibility[a] = 0.0;  // Zero out small traces
         }
      }
   }

   // Update visit count and average reward
   qTable[stateIdx].visitCount++;
   double oldAvg = qTable[stateIdx].avgReward;
   int n = qTable[stateIdx].visitCount;
   qTable[stateIdx].avgReward = oldAvg + (reward - oldAvg) / n;
   qTable[stateIdx].lastUpdate = (datetime)TimeCurrent();
}

//+------------------------------------------------------------------+
//| GetCurrentSessionBucket — Return session as integer 0-3          |
//+------------------------------------------------------------------+
int GetCurrentSessionBucket()
{
   ...
   if(hour >= 16 && hour < 24) return 2;    // NY
   return 3;  // Default
}
...
void NN_TrainOnTradeOutcome(const double &inputs[], double profit)
{
   ...
   // Target: +1 for profit, -1 for loss, 0 for breakeven
   double target = 0.0;
   if(profit > 0.5) target = 1.0;
   else if(profit < -0.5) target = -1.0;

   // Backprop with adaptive learning rate
   double lr = NN_LEARNING_RATE;
   if(MathAbs(profit) > 10.0) lr *= 1.5;  // Larger updates for significant outcomes

   NN_Backprop(inputs, target, lr);
}
...
+++ god_AGI_v8_Final_Complete.mq5 (修改后)
//+------------------------------------------------------------------+
//|         GOLDHUNTER ULTIMATE v8.4 UHF AGI — PHASE 4 EDITION            |
//|    Ultra-High-Frequency AGI Trading Bot for XAUUSD (Gold) Scalp/Swing    |
//|    Standard: MetaEditor 5 / XM Global MT5 Servers    |
...
int OnInit()
{
   ...
   Print("GoldHunter UHF AGI v8.4 Phase 5 initialized. Symbol: ", Symbol(),
         " | Magic: ", EA_MAGIC_NUMBER,
         " | Agents: 5 Specialists + VETO | DAG: 5-Node Execution | AGI: ACTIVE");
   return INIT_SUCCEEDED;
}
